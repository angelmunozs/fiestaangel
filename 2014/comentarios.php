...
<?php
$queryids = mysql_query("SELECT id FROM 2014_07_19_comentarios WHERE deleted=0 ORDER BY creado DESC");
$queryautores = mysql_query("SELECT usuario FROM 2014_07_19_comentarios WHERE deleted=0 ORDER BY creado DESC");
$querycomentarios = mysql_query("SELECT comentario FROM 2014_07_19_comentarios WHERE deleted=0 ORDER BY creado DESC");
$queryfechas = mysql_query("SELECT creado FROM 2014_07_19_comentarios WHERE deleted=0 ORDER BY creado DESC");
$querypadres = mysql_query("SELECT parent FROM 2014_07_19_comentarios WHERE deleted=0 ORDER BY creado DESC");
$querynombres = mysql_query("SELECT 2014_07_19_invitados.nombre AS nombre FROM 2014_07_19_invitados JOIN 2014_07_19_comentarios ON 2014_07_19_invitados.usuario = 2014_07_19_comentarios.usuario WHERE 2014_07_19_comentarios.deleted=0 ORDER BY 2014_07_19_comentarios.creado DESC");
// Ids
$ids = array();
...
if(mysql_num_rows($queryids)<1){
	...
	for($j=0;$j<mysql_num_rows($querycomentarios);$j++){
		$respondeA="";
		if($padres[$j]!=0){
			$aQuienResponde = mysql_query("SELECT usuario FROM 2014_07_19_comentarios WHERE id='".$padres[$j]."'");
			$respondidos = array();
...
if(isset($_POST['submit'])){
	if(!$_POST['comentario']){
		...
		$nuevo_parent = 0;
		$querynuevocomment = "INSERT INTO 2014_07_19_comentarios (usuario, comentario, creado, parent) VALUES ('".$nuevo_usuario."', '".$nuevo_comentario."', '".$nuevo_creado."', '".$nuevo_parent."')";
...
if(isset($_POST['borrarComment'])){
	if(!$_POST['laId']){
		...
		$queryborrarcomment = "UPDATE 2014_07_19_comentarios SET deleted=1 WHERE id='$antiguo_comment'";
